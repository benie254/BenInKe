...
export class EditChapterComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    // this.chapterForm.push(this.description.control(''));
  ...
  itemDetails(){
    ...
    this.storiesService.getChapDetails(this.selected).subscribe({
      next: (res) => {
        ...
        this.details = res;
        this.chapDetails = JSON.parse(this.details.description);
        console.warn("chap det",this.chapDetails)
        this.htmlContent = this.details.description
      }
    })
  }
